import { API_CONFIG, REQUEST_TIMEOUT_MS, MAX_RETRY_ATTEMPTS, RETRY_DELAY_MS } from '@payments-view/constants';

import type { ILogger } from '@payments-view/domain';

import { createLogger } from '../logging';
import type { ApiErrorResponse, ApiResult } from '../gnosis-pay/types';
...
export class CoinGeckoClient {
  ...
  constructor(baseUrl?: string, apiKey?: string, logger?: ILogger) {
    this.baseUrl = baseUrl ?? API_CONFIG.COINGECKO.BASE_URL;
    this.apiKey = apiKey;
    this.logger = logger ?? createLogger({ service: 'CoinGeckoClient' });
  }
  ...
  async request<T>(endpoint: string, options: RequestOptions = {}): Promise<ApiResult<T>> {
    const { method = 'GET', body, timeout = REQUEST_TIMEOUT_MS, retries = MAX_RETRY_ATTEMPTS } = options;
    ...
    this.logRequestStart(ctx.correlationId, method, ctx.url, Boolean(body));

    return this.executeWithRetry<T>(ctx, method, body, timeout, retries);
  }

  private logRequestStart(correlationId: string, method: string, url: string, hasBody: boolean): void {
    this.logger.info('API request started', { correlationId, method, url: this.sanitizeUrl(url), hasBody });
  }
  ...
  private sanitizeUrl(url: string): string {
    try {
      ...
      const sensitiveParams = ['apiKey', 'key', 'secret'];
